...
function updateCategoryTabCounts() {
  ...
  giftsCatalog.forEach((gift) => {
    if (counts[gift.category] !== undefined) {
  ...
  const tabLabels = {
    ...
    gacha: `🎲 Gacha (${counts.gacha})`,
    event: `🐉 Sự Kiện (${counts.event})`,
    mob: `🧟 Mob & Boss (${counts.mob})`,
    item: `🍎 Vật Phẩm (${counts.item})`,
    trap: `💣 Bẫy (${counts.trap})`,
  };
