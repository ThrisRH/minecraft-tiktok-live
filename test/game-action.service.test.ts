...
test("summons zombie and shows title/chat per 50-like milestone", async () => {
  ...
  assert.ok(
    ...
    "Chat message should be sent on hitting 50 milestone",
  );
  assert.ok(
    commands.some((command) => command.includes("User1")),
    "Participant name should be displayed on hitting 50 milestone",
  );
  ...
  assert.ok(
    ...
    "User2 should independently trigger 50-like milestone zombie",
  );
});
